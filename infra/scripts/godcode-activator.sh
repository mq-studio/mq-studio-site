#!/bin/bash
# GodCode Thinking Protocol Activator
# Enables comprehensive thinking mode for Claude interactions

GODCODE_PROTOCOL="/home/ichardart/code/infra/config/godcode-thinking-protocol.txt"
GODCODE_ACTIVE_FLAG="/tmp/godcode-active"

# Create protocol file
cat > "$GODCODE_PROTOCOL" << 'EOF'
...
echo "✅ GodCode thinking protocol created at: $GODCODE_PROTOCOL"

# Create Claude Code integration
echo "🔧 Setting up Claude Code integration..."

# Add alias to bashrc for easy activation
if ! grep -q "godcode-activate" ~/.bashrc; then
    cat >> ~/.bashrc << 'EOF'

# GodCode Thinking Protocol Activation
alias godcode-activate='echo "🧠 GodCode Thinking Protocol ACTIVATED" && touch /tmp/godcode-active && export GODCODE_MODE=true'
alias godcode-deactivate='echo "🧠 GodCode Thinking Protocol DEACTIVATED" && rm -f /tmp/godcode-active && unset GODCODE_MODE'
alias godcode-status='if [[ -f /tmp/godcode-active ]]; then echo "🧠 GodCode: ACTIVE"; else echo "🧠 GodCode: INACTIVE"; fi'

# Auto-prepend GodCode protocol when active
godcode-prompt() {
    if [[ -f /tmp/godcode-active ]]; then
        echo "$(cat /home/ichardart/code/infra/config/godcode-thinking-protocol.txt)"
        echo ""
        echo "$@"
    else
        echo "$@"
...
EOF
    echo "✅ Claude Code aliases added to ~/.bashrc"
fi

# Create wrapper script for Claude Code
cat > /home/ichardart/code/infra/scripts/claude-with-godcode.sh << 'EOF'
#!/bin/bash
...
chmod +x /home/ichardart/code/infra/scripts/claude-with-godcode.sh

echo "✅ Claude Code wrapper script created"
echo ""
echo "📋 Claude Code Setup Complete:"
echo "  - Type: godcode-activate (activates mode)"
echo "  - Type: godcode-deactivate (deactivates mode)"
echo "  - Type: godcode-status (check status)"
echo "  - When active, all Claude interactions use comprehensive thinking"