...
- **Application Entry:** `app/` directory contains Next.js App Router pages, with `app/layout.tsx` defining global providers and `app/globals.css` loading shared styles
- **Components:** Reusable UI components sit under `components/`, while `src/` contains design tokens (`src/design/`), components, and utilities surfaced through `src/index.ts`
- **Assets & Documentation:** Background assets and design references stay in `background/`; authored prompts/specifications are tracked in `specs/` and `templates/`
- **Scripts & Tests:** Automation scripts reside in `scripts/`; browser automation tests under `tests/stagehand` and unit tests under `tests/unit`
- **Python Utilities:** Python utilities and parity checks live in `python_utils/` and `python_tests/`
...
## Testing Guidelines
Co-locate unit tests in `tests/unit` using Jest and Testing Library; name files `<component>.unit.test.tsx`. Browser regression tests belong in `tests/stagehand`, and should import shared helpers from `tests/stagehand/axe-helper.js` for accessibility assertions. Playwright tests live in `tests/playwright/`. Python parity checks live in `python_tests/`; keep them passing when touching cross-language logic.
